...
import locals from './AlertingChartOverlay.mless';

const userInteractionThrottlingMillis = 50;

export default connectTo(
  props => {
    const { chart, timeConfig } = props;
    const observables = {
      highlightedMoment: highlightedMoment$,
      events: chart.chartEventsManager.getAlertsSubscription(props).startWith([])
    };
  ...
  class extends React.Component {
    static displayName = 'AlertingChartOverlay';
        ...
        if (nearestTimeInMetrics) {
          ...
          tooltipContent = (
            <TooltipLineAndContent
              ...
              customEventSection={AlertEventSection}
            />
          );
        }
      }
