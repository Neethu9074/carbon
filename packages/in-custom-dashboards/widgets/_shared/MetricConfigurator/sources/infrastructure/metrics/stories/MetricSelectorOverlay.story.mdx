import { Meta, Story, Preview, Props, ArgsTable } from '@storybook/addon-docs';
import { action as storybookAction } from '@storybook/addon-actions';

import MetricSelectorOverlay from 'in-custom-dashboards/widgets/_shared/MetricConfigurator/sources/infrastructure/metrics/MetricSelectorOverlay';
import PermanentlyVisibleOverlay from 'in-components/overlays/OverlayPresenter/stories/PermanentlyVisibleOverlay';
import metricCatalogExampleJson from './metricCatalogExample';

<Meta title="auto-generated" component={MetricSelectorOverlay}
      title="MetricSelectorOverlay"
/>

export const Template = (args) =><PermanentlyVisibleOverlay><MetricSelectorOverlay {...args}
    onChange={storybookAction('onChange')}
    close={storybookAction('close')}
    onQueryChange={storybookAction('onQueryChange')}
/></PermanentlyVisibleOverlay>

## Metric Selector

This overlay can be used within the query builder to allow selection of a metric by navigating through
the MetricTree.

<Preview>
  <Story name="default" args={{
        metricCatalog: metricCatalogExampleJson,
        loading: false,
  }}>
    {Template.bind({})}
  </Story>
</Preview>

export function generateEmptyResult() {
  return {"tree": []};
}

export const emptyResult = generateEmptyResult();

<Preview>
  <Story name="WithNoResults"
         args={{
           metricCatalog: emptyResult,
        loading: false
         }}
    /*
  <ArgsTable of={MetricSelectorOverlay} />
*/
  >
    {Template.bind({})}
  </Story>
</Preview>

### Props
