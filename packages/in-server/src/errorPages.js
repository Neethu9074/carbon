...
exports.send404 = function send404(req, res) {
  ...
  res.status(404).set('Content-Security-Policy', csp).send(template404({}));
};

exports.send500 = function send500(req, res) {
  res.status(500).set('Content-Security-Policy', csp).send(template500({}));
};

exports.sendMaintenance = function sendMaintenance(req, res) {
  res.status(503).set('Content-Security-Policy', csp).send(templateMaintenance({}));
};
