...
export default function NumberBarOverlayBehavior(props) {
  const {
    close,
    setTagFilters,
    tagFilters,
  ...
  const [{ form }, onChange] = useState(() => getInitialState({ tagFilters, tag, showRange, showEquality }));

  useEffect(() => {
    onChange({ form: getInitialState({ tagFilters, tag, showRange, showEquality }).form });
  }, [tagFilters, tag, showRange, showEquality]);
...
function getInitialState({ tagFilters, tag, showRange, showEquality }) {
  const { gt, lt, gte, lte, neq, eq } = getNumberTagFilters({ tagFilters, tag, showRange, showEquality });
