...
const hostViewCols = [
  {
    title: t('in-internal:monitoringUnit.log.writer.host'),
    type: 'snapshotLink',
    typeArgs: {
  ...
  {
    title: t('in-internal:monitoringUnit.log.writer.hostCpuLoad'),
    type: 'metric',
    typeArgs: {
      getSnapshotId(row) {
        return row.host.get('id');
  ...
  {
    title: t('in-internal:monitoringUnit.log.writer.clickHouse.queryErrorRate'),
    type: 'metric',
    typeArgs: {
      getSnapshotId(row) {
...
export default connectTo(
  ...
  function LogWriter({ rows, timeConfig }) {
    ...
    return (
      <div>
        <h1>{t('in-internal:monitoringUnit.log.writer.title')}</h1>

        <DashboardSection title={t('in-internal:monitoringUnit.log.writer.hostCpuLoad')}>
          <Chart
          ...
          <DashboardSection title={t('in-internal:monitoringUnit.log.writer.incoming')}>
            <Chart
              ...
              y1={{
                ...
                metrics: rows.map(() => `metrics.meters.KPI.incoming.log_messages.calls`),
                labels,
                type: 'stackedArea'
              }}
            />
          ...
          <DashboardSection title={t('in-internal:monitoringUnit.log.writer.write')}>
            <Chart
              ...
              y1={{
                min: 0,
                formatter: number.perSecond.compact,
                metrics: rows.map(() => `metrics.meters.batching.logs.transmissions.calls`),
                labels,
                type: 'stackedArea'
              }}
            />
        ...
        <Columize>
          ...
          <DashboardSection title={t('in-internal:monitoringUnit.log.writer.logCleanup.skips.mutations')}>
            <Chart
              snapshotIds={rows.map(r => r.dropwizard.get('id'))}
              timeConfig={timeConfig}
              y1={{
        ...
        <Columize>
          <DashboardSection title={t('in-internal:monitoringUnit.log.writer.logColdTier.success')}>
            <Chart
              snapshotIds={rows.map(r => r.dropwizard.get('id'))}
              timeConfig={timeConfig}
              y1={{
                min: 0,
                formatter: number.compact,
                metrics: rows.map(() => `metrics.meters.logColdTierService.number-partitions-moved-success`),
                labels: labels,
                type: 'line'
              }}
            />
          ...
          <DashboardSection title={t('in-internal:monitoringUnit.log.writer.logColdTier.failure')}>
            <Chart
              snapshotIds={rows.map(r => r.dropwizard.get('id'))}
              timeConfig={timeConfig}
              y1={{
                min: 0,
                formatter: number.compact,
                metrics: rows.map(() => `metrics.meters.logColdTierService.number-partitions-moved-failure`),
                labels: labels,
                type: 'line'
              }}
            />
          ...
          <DashboardSection title={t('in-internal:monitoringUnit.log.writer.logColdTier.duration')}>
            <Chart
              snapshotIds={rows.map(r => r.dropwizard.get('id'))}
              timeConfig={timeConfig}
              y1={{
        ...
        <Columize>
          <DashboardSection title={t('in-internal:monitoringUnit.log.writer.logColdTier.skips.mutations')}>
            <Chart
              snapshotIds={rows.map(r => r.dropwizard.get('id'))}
              timeConfig={timeConfig}
              y1={{
          ...
          <DashboardSection title={t('in-internal:monitoringUnit.log.writer.logColdTier.skips.merges')}>
            <Chart
              snapshotIds={rows.map(r => r.dropwizard.get('id'))}
              timeConfig={timeConfig}
              y1={{
...
function getRowDetails(row) {
  return (
    <Fragment>
      <DashboardSection title={t('in-internal:monitoringUnit.log.writer.hostLoad')}>
        <Chart
          ...
          y1={{
            ...
            metrics: ['load.1min'],
            labels: [t('in-internal:monitoringUnit.log.writer.hostCpuLoad')],
            type: 'stackedArea'
          }}
        />
      </DashboardSection>

      <DashboardSection title={t('in-internal:monitoringUnit.log.writer.clickHouse.queryQueueing')}>
        <Chart
          ...
          y1={{
            ...
            labels: [t('in-internal:monitoringUnit.log.writer.clickHouse.newlyQueuedCalls')],
            type: 'line'
          }}
          y2={{
            min: 0,
            ...
            labels: [t('in-internal:monitoringUnit.log.writer.clickHouse.queuedCalls')],
            type: 'line'
          }}
        />
      </DashboardSection>

      <DashboardSection title={t('in-internal:monitoringUnit.log.writer.clickHouse.queryLatency')}>
        <Chart
          snapshotId={row.dropwizard.get('id')}
          timeConfig={row.timeConfig}
          margins={{
          ...
          y1={{
            ...
            labels: [t('in-internal:monitoringUnit.log.writer.clickHouse.calls')],
            type: 'stackedArea'
          }}
          y2={{
            formatter: millis.fixedCompact,
            ...
            labels: [
              t('in-internal:monitoringUnit.log.writer.clickHouse.mean'),
              t('in-internal:monitoringUnit.log.writer.clickHouse.50th'),
              t('in-internal:monitoringUnit.log.writer.clickHouse.99th')
            ],
            type: 'line'
          }}
        />
      ...
      {row.jvm.getIn(['data', 'jvm.collectors']) ? (
        <DashboardSection title={t('in-internal:monitoringUnit.log.writer.garbageCollection')}>
          <Chart
