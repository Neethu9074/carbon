...
import { ResourceImpactEntities, AgentSnapshot, ImpactedApplicationDetails, TurboActionCategory } from '@instana/types';
...
export default function DetailsModal1({ action, agents, targetSnapshotId, eventId }: DetailsModalProps) {
          ...
          <Stack direction="vertical">
            {error ? (
              <Trans
                ...
                values={{ actionName: action.name, message: data.errorMessage }}
  ...
  function handleRunAction() {
    ...
    runResourceOptimizationAction(params).once(data => {
      ...
      if ('errorMessage' in data && data.errorMessage != null) {
        ...
        setRunActionResponseId(data.actionInstanceId);
        runOptimizationTrackerSegment({ actionName: action.name, source: 'Turbonomic' }); //Turbonomic only for now and we may need to have a source parameter
        close();
        showToastNotification(data);
