...
describe('data', () => {
  ...
  it('must insert new data columns', () => {
    const newColumns = [column([[0, 3], [0, 2], [0, 1]])];
  ...
  it('must merge data columns and overwrite previously missing data points', () => {
    ...
    expect(data.getDataColumns()).to.deep.equal([column([[0, 3], [0, 4], [0, 1]])]);
  });

  it('must merge by overwriting previously existing values', () => {
    data.insertSorted([column([[0, 3], [0, 2], [0, 1]])]);
    data.insertSorted([column([null, [0, 4], null])]);
    expect(data.getDataColumns()).to.deep.equal([column([[0, 3], [0, 4], [0, 1]])]);
  });

  it('must add new data columns to the beginning', () => {
    data.insertSorted([column([[1, 3], [1, 2], [1, 1]])]);
    data.insertSorted([column([null, [0, 4], null])]);
    expect(data.getDataColumns()).to.deep.equal([column([null, [0, 4], null]), column([[1, 3], [1, 2], [1, 1]])]);
  });

  it('must add new data columns to the end', () => {
    data.insertSorted([column([[1, 3], [1, 2], [1, 1]])]);
    data.insertSorted([column([null, [2, 4], null])]);
    expect(data.getDataColumns()).to.deep.equal([column([[1, 3], [1, 2], [1, 1]]), column([null, [2, 4], null])]);
  });

  it('must add new data columns inbetween', () => {
    data.insertSorted([column([[1, 3], [1, 2], [1, 1]]), column([[3, 4], [3, 5], [3, 6]])]);
    data.insertSorted([column([null, [2, 4], null])]);
    expect(data.getDataColumns()).to.deep.equal([
      column([[1, 3], [1, 2], [1, 1]]),
  ...
  describe('expireDataPointsOlderThan', () => {
    it('must remove old data points', () => {
      ...
      expect(data.getDataColumns()).to.deep.equal([column([[3, 4], [3, 5], [3, 6]])]);
    });
