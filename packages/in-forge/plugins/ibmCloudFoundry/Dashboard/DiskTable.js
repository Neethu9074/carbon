...
const cols = [
  ...
  },
  {
    ...
    typeArgs: {
      ...
      getMetricName(row) {
        return `instances.${row.name}.app_disk_bytes_used`;
      },
  ...
  },
  {
    ...
    typeArgs: {
      ...
      getMetricName(row) {
        return `instances.${row.name}.app_disk_bytes_total`;
      },
      getContent: bytes.detailed,
      getTimeWindowAggregation() {
        return 'mean';
...
export default function DiskTable({ snapshot, timeConfig, instanceCount }) {
  ...
  return (
    <Table
      withoutPadding
      cardTitle={t('in-forge:plugins.ibmCloudFoundry.titleDisk')}
      cols={cols}
      rows={rows}
  ...
  return (
    <Chart
      ...
      y1={{
        ...
        metrics: ['instances.' + row.name + '.app_disk_bytes_used', 'instances.' + row.name + '.app_disk_bytes_total'],
