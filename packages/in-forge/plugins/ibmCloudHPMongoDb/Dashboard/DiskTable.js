...
const cols = [
  ...
  {
    title: t('in-forge:plugins.ibmCloudHPMongoDb.usedPercent'),
    type: 'sparkChart',
    typeArgs: {
      getSnapshotId(row) {
  ...
  {
    title: t('in-forge:plugins.ibmCloudHPMongoDb.mount'),
    type: 'string',
    typeArgs: {
      getValue(row) {
        return row.mount;
...
export default function DiskTable({ snapshot, timeConfig, nodeIds, mounts }) {
  ...
  return (
    <Table
      withoutPadding
      cardTitle={t('in-forge:plugins.ibmCloudHPMongoDb.titleDisk')}
      cols={cols}
      rows={rows}
  ...
  return (
    <Chart
      ...
      y1={{
        ...
        labels: [t('in-forge:plugins.ibmCloudHPMongoDb.usedPercent')],
        type: 'line'
      }}
