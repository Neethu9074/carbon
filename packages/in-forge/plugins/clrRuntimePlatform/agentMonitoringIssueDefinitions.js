  ...
  clr_env_var_not_defined: {
    issueDescription: {
      Component: function clrEnvVarNotDefined({ missingEnvKeys }) {
        ...
        return (
          <span>
            The process environment for this .NET Full Framework application is not correctly configured for Instana to be able to
    ...
    issueDescription: {
      Component: function clrEnvVarInvalidValue({ invalidEnvKey, invalidEnvValue, competitor }) {
        if (competitor) {
          return (
            <span>
              It seems that another tool is monitoring this .NET Full Framework process, likely {competitor}. The Instana host
