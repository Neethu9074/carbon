...
const cols = [
  ...
  {
    title: t('in-forge:plugins.oracleDB.bufferCacheHit'),
    type: 'metric',
    typeArgs: {
      ...
      getMetricName(row: Row) {
        return `stats.systemStats.${row.key}.bufferCacheHitRatio`;
      },
...
export default function DatasourcesTable({ snapshot, timeConfig }: DatasourcesTableProps) {
  ...
  return (
    <Table
      withoutPadding
      cardTitle={t('in-forge:plugins.oracleDB.bufferCacheHit', {
        len: rows.length
      })}
      cols={cols}
      rows={rows}
    ...
    <div>
      <Chart
        ...
        y1={{
          ...
          metrics: [`stats.systemStats.${row.key}.bufferCacheHitRatio`],
