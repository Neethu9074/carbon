...
export function HttpSpanDetailViewDescriptionList({ span }) {
  ...
  return (
    <Fragment>
      ...
      <Di title={t('in-forge:tracing.http.titleHystrixFallbackURI')}>{span.getIn(['data', 'http', 'hystrix_fallback_uri'])}</Di>
      {params != null && <Di title={t('in-forge:tracing.http.titleParameters')}>{isBlank(params) ? '<no query parameters>' : params}</Di>}
      ...
      <Di title={t('in-forge:tracing.http.titleContentLength')}>{span.getIn(['data', 'http', 'size'], span.getIn(['data', 'net', 'in']))}</Di>
