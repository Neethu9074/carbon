...
export default function useTagCatalog({ metric, ownerType, includeMetricTags = false }: UseTagCatalogProps): TagCatalog | undefined {
    const timeConfig = useTimeConfig();

    const filter = { timeConfig, tagFilterExpression: EMPTY_EXPRESSION };

    const tagCatalogResult = useObservable(() => getTagCatalog({ filter, metric, ownerType, includeMetricTags }), [timeConfig, metric, ownerType, includeMetricTags]);
