...
import { carbonPrefix } from 'in-infrastructure/GraphExplorer/charts-react/utils';
...
> = ({
  ...
  const subtitleElement = subtitle ? <div className={`${namespace}__subtitle`}>{subtitle}</div> : null;
  const descriptionElement = description ? <div className={`${namespace}__description`}>{description}</div> : null;

  return (
    <Component className={circleClasses} style={{ height: size, width: size, position }} tabIndex={0} {...rest}>
