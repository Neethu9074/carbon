...
import { ThumbsFeedbackObject, TypeTextObject, TableChartObject, EventsTableObject } from 'in-aichat/ResponseObjects';
import { formatCarbonDate, formatCarbonTime } from 'in-events/components/util/carbonDateTimeFormat';
import { Entry, TableRow, TableHeader } from 'in-aichat/TableComponents/useTableState';
...
export function formatForTable(apiResponse: any, userQuery: string, queryResponse: any) {
  ...
  const headerAlias: { [key: string]: string } = {
    'label.kubernetesNode': t('in-aichat:aichat.host'),
    'latency.mean': t('in-aichat:aichat.meanLatency'),
    'calls.sum': t('in-aichat:aichat.numCalls')
  };
  const emptyResult = {
    output: {
      generic: [
  ...
  if (first.name) {
    response.data.headers.push({ key: 'name', header: t('in-aichat:aichat.name') });
  } else if (useTag) {
  ...
  if (firstMetric.length === 2) {
    ...
    if (countPresent) {
      response.data.headers.push({ key: 'count', header: t('in-aichat:aichat.count') });
    }
    response.data.headers.push({ key: 'timestamp', header: t('in-aichat:aichat.timestamp') });
...
export function formatForEventsTable(apiResponse: any, userQuery: string, queryResponse: any) {
  ...
  // Standard headers
  const baseHeaders = [
    { key: 'name', header: t('in-aichat:aichat.name') },
    { key: 'on', header: t('in-aichat:aichat.on') },
    { key: 'started', header: t('in-aichat:aichat.started') },
    { key: 'end', header: t('in-aichat:aichat.end') },
    { key: 'state', header: t('in-aichat:aichat.state') }
  ];

  // Headers with group column
  const groupedHeaders = [
    ...baseHeaders.slice(0, 2),
    { key: 'group', header: t('in-aichat:aichat.group') },
    ...baseHeaders.slice(2)
  ];
