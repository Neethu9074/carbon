...
const TableFilterPopover = ({
  ...
  className,
  filterOptions = [],
  filterLabel = t('in-aichat:aichat.group'),
  selectedValues = [],
  tableId
    ...
    <Layer>
      <Popover open={isOpen} isTabTip onRequestClose={() => setIsOpen(false)} align={align}>
        <IconButton
          ...
          label={t('in-aichat:aichat.filtering')}
        >
          <Filter />
        </IconButton>
        <PopoverContent
        ...
        >
          ...
          <div className={classNames(`${prefix}--filter-actions`, locals.filterActions)}>
            <Button
              kind="secondary"
              title={t('in-aichat:aichat.resetFilters')}
              onClick={handleResetFilter}
              className={locals.resetButton}
              id={`reset-btn-${tableId}`}
            >
              {t('in-aichat:aichat.resetFilters')}
            </Button>
            <Button
              kind="primary"
              title={t('in-aichat:aichat.applyFilter')}
              onClick={handleApplyFilter}
              className={locals.applyButton}
              id={`apply-btn-${tableId}`}
            >
              {t('in-aichat:aichat.applyFilter')}
            </Button>
          </div>
        </PopoverContent>
      </Popover>
