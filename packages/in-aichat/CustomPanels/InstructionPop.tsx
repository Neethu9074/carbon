...
import { AI_CHAT_TAG_NAME, WAC_WIDGET } from 'in-aichat/utils/utils';
...
const InstructionPop = ({ setInstructionPopOpen }: InstructionPopProps) => {
  ...
  return (
    ...
    >
      <div className={locals.heading}>{t('in-aichat:aichat.prompting')}</div>
      <div className={locals.body}>
        {t('in-aichat:aichat.editSample')}
        <Button
          onClick={() => {
