...
import ElkForm from 'in-settings/tabs/GlobalSettings/pages/integrations/logging/Elk/ElkForm';
import { globalSettingsIntegrationsLoggingElk } from 'in-settings/navigation/paths';
...
const Elk = () => {
  ...
  const loadConfiguration = useCallback(() => {
    ...
    errorSubscriptionRef.current = result$.errors().once(() => {
      setLoading(false);
      setMessage(t('in-settings:tabs.failedToLoadElkConfiguration'));
    });
  }, [disposeAsyncAction]);
  ...
  const saveIntegrationForm = useCallback(() => {
    ...
    responseSubscriptionRef.current = result$.once(() => {
      ...
      goToPath(globalSettingsIntegrationsLoggingElk);
    });
