...
import './AddNewAlertChannelDialog.less';

const block = 'in-alerting-add-new-alert-channel-dialog';

export default class extends React.Component {
  static displayName = 'AddNewAlertChannelDialog';
  ...
  render() {
    ...
    const steps = [
      <div className={block}>
        <AlertChannelSwitch onClick={selectedType => this.setState({ selectedType })} selectedType={selectedType} />
      </div>,
  ...
  onSave = () => {
    const alertChannel = fromJS(configs[this.state.selectedType].createEntity(null, this.state.form));
    const result$ = saveIntegration(alertChannel);
    ...
    this.responseSubscription = result$.once(() => {
      this.props.onClose(alertChannel);
      close();
    });
