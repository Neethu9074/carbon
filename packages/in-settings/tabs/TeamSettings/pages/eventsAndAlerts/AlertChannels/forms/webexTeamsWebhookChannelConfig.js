...
const name = 'WEBEX_TEAMS_WEBHOOK';
const label = 'Cisco Webex Teams Webhook';
      ...
      {form.get('name').map(field => (
        <FormGroup className={block}>
          ...
          <Input
            ...
            type="text"
            placeholder="Cisco Webex Teams Alert Channel"
            value={field.value}
      ...
      {form.get('webhookUrl').map(field => (
        <FormGroup>
          <Label htmlFor="webhookUrl" hasError={!field.valid && field.touched}>
            Webhook URL to Cisco Webex Teams{' '}
            <a href="https://apphub.webex.com/teams/applications/incoming-webhooks-cisco-systems">Incoming Webhook</a>
          </Label>
          <Input
            className={`${block}__input`}
            id="webhookUrl"
