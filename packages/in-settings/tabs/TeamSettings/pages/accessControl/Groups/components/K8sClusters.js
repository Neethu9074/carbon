...
import getKubernetesClusters from 'in-kubernetes/subscriptions/getKubernetesClusters';
...
const columnDefinitions = [
  ...
  },
  {
    getContent({ item }) {
      return (
        <Tooltip content={item.cluster.label} align="topLeft" delay={500}>
          <div>{item.cluster.label}</div>
        </Tooltip>
      );
    }
  }
