...
import locals from './MobileApps.mless';
...
function defaultGetHeader(inSelectListDialog, tableActions) {
  return leftHeaderWithSelectAll('Access Mobile Apps', inSelectListDialog, tableActions);
}

function getEntityName(entity) {
  return `Mobile Apps "${entity.name}"`;
}

export function noRightHeader() {
  // Used to explicitly disable that default right header when this is used in a
  // dialog to select mobile apps in the access permission set form. Reason: The create-new button would navigate away from
