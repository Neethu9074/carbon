...
const cols = [
  ...
  },
  {
    ...
    typeArgs: {
      ...
      getMetricName(row) {
        return `micrometer.metrics.timeGauge.${row.name}`;
      },
...
export default function TimeGauge({ snapshot, timeConfig, titlePrefix }) {
  ...
  const title = (titlePrefix ? titlePrefix : '') + ' Time Gauges (' + rows.length + ')';
  return (
  ...
  return (
    <Chart
      ...
      y1={{
        ...
        metrics: ['micrometer.metrics.timeGauge.' + row.name],
        labels: [row.name],
        type: 'line'
      }}
    />
