...
const cols = [
  ...
  },
  {
    ...
    typeArgs: {
      ...
      getMetricName(row) {
        return `micrometer.metrics.gauge.${row.name}`;
      },
...
export default function MicrometerGauges({ snapshot, timeConfig, titlePrefix }) {
  ...
  const title = (titlePrefix ? titlePrefix : '') + ' Gauges (' + rows.length + ')';
  return (
  ...
  return (
    <Chart
      ...
      y1={{
        ...
        metrics: ['micrometer.metrics.gauge.' + row.name],
        labels: [row.name],
        type: 'line'
      }}
    />
