...
const cols = [
  ...
  },
  {
    ...
    typeArgs: {
      ...
      getMetricName(row) {
        return `micrometer.metrics.counter.${row.name}`;
      },
...
export default function MicrometerCountersTable({ snapshot, timeConfig, titlePrefix }) {
  ...
  const title = (titlePrefix ? titlePrefix : '') + ' Counters (' + rows.length + ')';
  return (
  ...
  return (
    <Chart
      ...
      y1={{
        ...
        metrics: ['micrometer.metrics.counter.' + row.name],
        labels: [row.name],
        type: 'line'
      }}
    />
