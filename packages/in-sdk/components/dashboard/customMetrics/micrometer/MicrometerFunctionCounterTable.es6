...
const cols = [
  ...
  },
  {
    ...
    typeArgs: {
      ...
      getMetricName(row) {
        return `micrometer.metrics.functionCounter.${row.name}`;
      },
...
export default function FunctionCounter({ snapshot, timeConfig, titlePrefix }) {
  ...
  const title = (titlePrefix ? titlePrefix : '') + ' Function Counters (' + rows.length + ')';
  return (
  ...
  return (
    <Chart
      ...
      y1={{
        ...
        metrics: ['micrometer.metrics.functionCounter.' + row.name],
        labels: [row.name],
        type: 'line'
      }}
    />
