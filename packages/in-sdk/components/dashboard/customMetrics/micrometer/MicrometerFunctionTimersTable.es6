...
const cols = [
  ...
  },
  {
    ...
    typeArgs: {
      ...
      getMetricName(row) {
        return `micrometer.metrics.functionTimer.${row.name}`;
      },
...
export default function MetersTable({ snapshot, timeConfig, titlePrefix }) {
  ...
  const title = (titlePrefix ? titlePrefix : '') + ' Function Timers (' + rows.length + ')';
  return (
  ...
  return (
    <Chart
      ...
      y1={{
        ...
        metrics: ['micrometer.metrics.functionTimer.' + row.name],
        labels: ['value'],
        type: 'line'
      }}
    />
