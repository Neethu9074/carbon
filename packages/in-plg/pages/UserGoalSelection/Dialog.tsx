...
import { Goal } from 'in-plg/pages/UserGoalSelection/types';
...
const UserGoalSelectionDialog = () => {
  ...
  const isGoalSelected = (goal: Goal) => selectedGoals.find(selectedGoal => selectedGoal.id === goal.id);
  const goalSelectHandler = (checked: boolean, goal: Goal) => {
...
function submitHandler(selectedGoals: Goal[], otherGoal: string) {
...
function goalSetHandler(
  ...
  setSelectedGoals: Dispatch<SetStateAction<Goal[]>>
) {
  if (goal.type == TOGGLER) {
  ...
  setSelectedGoals((prevSelectedGoals: any) => {
    if (checked) {
      ...
      return prevSelectedGoals.filter((preGoal: Goal) => preGoal.id !== goal.id);
    }
  });
}
