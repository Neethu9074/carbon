...
(function(OCTREE) {
  ...
  function isArray(target) {
    ...
    // we are only inserting objects without length property, so this should do the trick
  ...
  OCTREE.ROctreeObjectData.prototype = {
    update: function() {
      ...
      if (this.vertices) {
        this.radius = object.material.size || 1;
        this.position.copy(this.vertices).applyMatrix4(object.matrixWorld);
      } else {
        if (object.geometry) {
          ...
          this.position.copy(object.geometry.boundingSphere.center).applyMatrix4(object.matrixWorld);
        } else {
          this.radius = object.boundRadius;
          this.position.setFromMatrixPosition(object.matrixWorld);
        }
      }

      this.radius *= Math.max(object.scale.x, object.scale.y, object.scale.z);
    }
  };
  ...
  OCTREE.OctreeNode.prototype = {
    ...
    addNode: function(node, indexOctant) {
      ...
      this.nodesByIndex.set(indexOctant, node);
      if (node.parent !== this) {
        node.setParent(this);
      }
    },
    ...
    removeNode: function(indexOctant) {
      ...
      node = node || this.nodesByIndex.get(indexOctant);

      this.nodesByIndex.delete(indexOctant);
    ...
    branch: function(indexOctant) {
      ...
      // node exists
      const nodeByIndex = this.nodesByIndex.get(indexOctant);
      if (nodeByIndex instanceof OCTREE.OctreeNode) {
        node = nodeByIndex;
      } else {
        // properties
    ...
    checkMerge: function() {
      ...
      while (
        nodeParent.parent instanceof OCTREE.OctreeNode &&
    ...
    checkContract: function() {
      var i, l, node, nodeObjectsCount, nodeHeaviest, nodeHeaviestObjectsCount, outsideHeaviestObjectsCount;
