...
describe('in-mobile-apps/analyze/SessionView/tabs/Summary/fixClockSkewProblems', () => {
  describe('fixClockSkewProblems', () => {
    ...
    it('must not require fixes when there is no session start as everything is the earliest beacon defines the order', () => {
      const beacons = [getBeacon('custom', 11000, 5600), getBeacon('httpRequest', 12000, 4800)];
      const result = fixClockSkewProblems(beacons);
      expect(result.requiredFixes).to.equal(false);
      expect(result.beacons).to.equal(beacons);
    });
    ...
    it('must fix the timestamps when the timestamps do not make sense', () => {
      const beacons = [
        ...
        getBeacon('sessionStart', 100000, 50000),
        getBeacon('custom', 110000, 50000)
      ];
      const result = fixClockSkewProblems(beacons);
      expect(result.requiredFixes).to.equal(true);
      expect(result.beacons).to.deep.equal([
        getBeacon('sessionStart', 50000, 0),
        getBeacon('custom', 60000, 0),
