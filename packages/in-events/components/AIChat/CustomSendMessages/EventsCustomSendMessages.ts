    ...
    output: {
      generic: [
        ...
        },
        {
          ...
          text: t('in-events:aichat.findingInfoFrom', { endpoint: endpoint })
        }
      ]
    }
  } as MessageResponse;
...
function processTraditionalQuery(instance: ChatInstance, userQuery: string) {
  ...
  sendAPIQuery(userQuery).once(
    (queryResponse: any) => {
      ...
      if (!queryResponse) {
        sendError(instance, t('in-events:aichat.noData'), queryResponse, userQuery);
        return;
      }
      ...
      if (!queryResponse.api?.api_endpoint) {
        sendError(instance, t('in-events:aichat.unableToFindError'), queryResponse, userQuery);
        return;
      }
    ...
    (queryError: any) => {
      ...
      if (msg.includes('HttpRequestTimeoutError') || msg.includes('Request timed out')) {
        sendError(instance, t('in-events:aichat.problemError'), null, userQuery);
      } else {
  ...
  const statusMessage = {
    ...
    output: {
      generic: [
        {
          response_type: MessageResponseTypes.TEXT,
          text: t('in-events:aichat.findingInfo')
        } as TextItem
      ]
    }
  } as MessageResponse;
