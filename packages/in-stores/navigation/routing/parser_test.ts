...
describe('in-stores/navigation/routing/parser', () => {
  describe('parseUrl', () => {
    ...
    it('must extract query parameters', () => {
      expect(parseUrl('/foo?a=b')).toEqual({
        pathname: '/foo',
        query: {
          a: 'b'
        },
    ...
    it('must extract multiple query parameters', () => {
      expect(parseUrl('/foo?a&b=c')).toEqual({
        pathname: '/foo',
        query: {
          a: '',
          b: 'c'
    ...
    it('must support matrix parameters', () => {
      expect(parseUrl('/foo;k=v')).toEqual({
        pathname: '/foo',
        query: {},
        matrix: {
          '/foo': {
    ...
    it('must support matrix parameters for sub paths', () => {
      expect(parseUrl('/first/second;k=v/third')).toEqual({
    ...
    it('must support matrix parameters with query parameters', () => {
      expect(parseUrl('/first;a=b/second;k=v/third?foo=bar')).toEqual({
        pathname: '/first/second/third',
        query: {
          foo: 'bar'
        },
    ...
    it('must not decode matrix parameters recursively', () => {
      expect(parseUrl('/abc;foo=a%252520b/bar')).toEqual({
        pathname: '/abc/bar',
        query: {},
        matrix: {
          '/abc': {
    ...
    it('must support table use case', () => {
      expect(parseUrl('/table;view=physical;plugin=host?timeline.to&timeline.ws=600000')).toEqual({
        pathname: '/table',
        query: {
          'timeline.to': '',
          'timeline.ws': '600000'
