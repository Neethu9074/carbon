...
// https://raw.githubusercontent.com/wmertens/jsurl/master/lib/jsurl2.js
//
// which is a fork of
// https://github.com/Sage/jsurl
//
// It was copied instead of being added to dependencies, because the maintainer is not
// well known and the project has just one star (from the maintainer). It was therefore
// deemed to dangerous to use automatic updates / NPM versions.
//
// I (Ben@2018-12-28) did a pass through the code for security issues.
// ####################################################################################
// ####################################################################################
...
function eat(a) {
  var j, c;
  for (j = a.i; j < a.l && ((c = a.s.charAt(j)), c !== '~' && c !== ')'); j++) {
    // pass
  }
...
function encode(v, out, rich, depth) {
  ...
  if (T === 'number') {
    out.push(isFinite(v) ? v.toString() : rich ? (isNaN(v) ? NAN : v > 0 ? INF : NINF) : NULL);
  } else if (T === 'boolean') {
    out.push(v ? '' : FALSE);
  } else if (T === 'string') {
    t = escape(v);
...
export function tryParse(s, def, options) {
  try {
