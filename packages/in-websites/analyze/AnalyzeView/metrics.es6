import { millis, percentage, bytes, number } from 'in-services/formatters/number';
...
function newSizeMetric(metric, label, category) {
  return {
    metric,
    label,
    formatter: bytes,
...
function newNumberMetric(metric, label, category) {
  return {
    metric,
    label,
