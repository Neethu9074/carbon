...
export default function Activity({ beacons, firstBeacon, activeBeaconTypeFilters }) {
  return (
    <Fragment>
      ...
      {beacons
        ...
        .map(beacon => (
          <Beacon beacon={beacon} earliestTimestamp={firstBeacon.timestamp} key={beacon.beaconId} />
        ))}
    </Fragment>
  );
}
