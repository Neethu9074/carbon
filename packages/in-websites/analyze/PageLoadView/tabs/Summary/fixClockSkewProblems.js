...
// Attempt to fix clock skews in user data. Clock skews occur when end-user browsers delay transmission
...
export function fixClockSkewProblems(beacons) {
  ...
  if (beaconWithEarliestTimestamp.timestamp >= pageLoad.timestamp) {
    return { beacons, requiredFixes: false };
  }
