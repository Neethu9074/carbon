...
import useSyntheticsEventBasedLatencyMetrics from 'in-service-levels/hooks/useSyntheticsLatencyEventBasedMetrics';
...
interface SyntheticsEventBasedLatencyIndicatorChartProps {
...
export default function SyntheticsEventBasedLatencyIndicatorChart({
  automaticallySize,
  customHeight,
  ...
  title
}: SyntheticsEventBasedLatencyIndicatorChartProps) {
  const sloZoomInAction = useSloZoomInAction();
  const timeConfig = useContextAwareSloTimeWindowConfig();
  const granularity = calculateEventGraphGranularity(timeConfig);
  const result = useSyntheticsEventBasedLatencyMetrics(indicator, entity, timeConfig, granularity);
