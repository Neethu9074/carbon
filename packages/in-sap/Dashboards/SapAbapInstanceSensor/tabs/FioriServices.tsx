...
export default function FioriServices({ snapshotId }: FioriServiceProps) {
  ...
  const rows: FioriRow[] = fioriDetails
    .filter((item: Map<string, object>) => item.has('serviceName'))
    .toArray()
  ...
  let items = rows.map(item => {
    return {
      label: item.fioriDetail.get('serviceName'),
