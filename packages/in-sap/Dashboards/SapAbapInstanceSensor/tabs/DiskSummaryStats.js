...
export default connectTo(
  props => {
    snapshotMap = props;
    return {
      data: getRawPayloadWithTimestamp(props.snapshotId, 'diskSummaryStats')
    };
  },
  function DiskSummaryStats({ data }) {
    if (!data) {
      return null;
    }
            ...
            <DashboardSection title={t('in-sap:dashboards.performanceStats')}>
              <Chart
                ...
                y1={{
                  min: 0,
                  metrics: [
                    'diskSummaryStats.' + row.key + '.avgQueueLength',
            ...
            <DashboardSection title={t('in-sap:dashboards.diskOperations')}>
              <Chart
                ...
                y1={{
                  min: 0,
                  metrics: [
                    'diskSummaryStats.' + row.key + '.kbPerSec',
                    'diskSummaryStats.' + row.key + '.operationsPerSec'
                  ],
