...
function positionCall(callFrames, call, parentCall, depth, traceStart, totalDuration, occupiedTimeRangesByDepth) {
  const { start, model, duration, children, ...props } = call;
  ...
  if (isFakeRoot) {
    ...
    depthWithoutOverlapping = findDepthWithoutAnyOverlapping(depth, model, [start, end], occupiedTimeRangesByDepth);
...
function findDepthWithoutAnyOverlapping(minDepth, model, timeRange, occupiedTimeRangesByDepth) {
  if (model === 'LOG') {
    return minDepth;
  }
