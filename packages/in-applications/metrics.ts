...
function isBlockSizeLessThanMinute(roundedBlockSize) {
  return roundedBlockSize < 1;
}

function isBlockSizeGreaterThanOrEqualHour(roundedBlockSize) {
  return roundedBlockSize >= 60;
}

function roundMillisToSeconds(blockSizeMillis) {
  return Math.round(blockSizeMillis / 1000);
}

function roundMillisToMinutes(blockSizeMillis) {
  return Math.round(blockSizeMillis / (1000 * 60));
}

function roundMillisToHours(blockSizeMillis) {
