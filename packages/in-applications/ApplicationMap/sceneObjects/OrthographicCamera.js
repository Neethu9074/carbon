...
export default class OrthographicCameraWrapper {
  ...
  update() {
    ...
    getServiceLocators(this.serviceLocatorUid).eventBusServiceLocator.emit(SIGNALS.CAMERA_UPDATE, this);
  }

  setCameraSize(value) {
    this.cameraSize = Math.max(1, value);
    if (!this.initialCameraSize) {
      this.initialCameraSize = this.cameraSize;
    }
