...
import fragmentShader from 'in-applications/ApplicationMap/misc/ParticleEmitter/shader/fragmentShader.glsl';
import vertexShader from 'in-applications/ApplicationMap/misc/ParticleEmitter/shader/vertexShader.glsl';
import calculate from 'in-applications/ApplicationMap/misc/ParticleEmitter/callsToParticlesCalculator';
...
const TTLS = [0.2, 0.24, 0.2, 0.2, 0.3, 0.2, 0.28, 0.15, 0.34, 0.2];
const OFFSETS = [-0.03, 0.04, 0.03, -0.024, -0.035, 0.028, -0.04, -0.032, 0.038, 0.029];
...
export default class ParticleEmitter {
  ...
  start() {
    ...
    this.updateSubscription = getServiceLocators(this.serviceLocatorUid)
      .eventBusServiceLocator.on(SIGNALS.UPDATE)
  ...
  update(dt) {
    ...
    for (let i = 0, length = particles.length; i < length; i++) {
      ...
      const timeToLife = particle.ttlPerUnit * this.length;
  ...
  spawnParticle(hasError) {
    ...
    const vertices = this.vertices;
    const position = { x: 0, y: OFFSETS[newIndex % OFFSETS.length], z: 0 };
    const particle = {
      progress: 0,
      timeLived: 0,
      index: newIndex,
      ttlPerUnit: TTLS[newIndex % TTLS.length]
    };
