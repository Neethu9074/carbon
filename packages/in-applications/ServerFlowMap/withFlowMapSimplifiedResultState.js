...
export default () => ComposedComponent => {
  return class StatefulFlowMapComponent extends Component {
    static displayName = getDisplayName(ComposedComponent, 'withFlowMapSimplifiedResultState');
    ...
    getFlowNodes$ = (nodeId, path, page, direction) => {
      ...
      let splitSeparator = incoming ? '<-' : '->';
      const split = nodeId.split(splitSeparator);
      let serviceId = split[incoming ? 0 : split.length - 1];
      const services = getServices({
        ...
        contextScope: direction === 'OUTGOING' ? 'DOWNSTREAM' : 'UPSTREAM'
      });
      return services;
    };
