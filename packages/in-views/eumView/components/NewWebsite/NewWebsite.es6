...
export default class NewWebsite extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  onSubmit = e => {
    ...
    this.saveSubscription = combineDataAndError(addKey(field.value)).once(({ data, error }) => {
      if (error) {
        ...
        this.snapshotSubscription = getSnapshot(data.websiteSnapshotId, getWaitForEntityCreationTimeConfig()).subscribe(
