...
import SensitivityDefaultChart from 'in-views/configurationView/tabs/TeamSettings/pages/knowledgeManagement/CustomDynamicRules/components/SensitivityDefaultChart';
        ...
        <div className={`${block}__chart-wrapper`}>
          ...
          {!forecastAvailable ? (
            <div className={`${block}__training-indicator`}>
              <SvgIcon className={`${block}__icon`} type="spinner" color="#fff" width={16} spinning />
              Training metric’s historical behavior...
            </div>
          ) : null}
        </div>
      );
  ...
  return (
    <div className={`${block}__legend`}>
      <div className={`${block}__legend-block`}>
        <div style={{ background: '#ff4229' }} className={`${block}__legend-rect`} />
        irregular metric trend
      </div>
      <div className={`${block}__legend-block`}>
        <div style={{ background: '#e5e5e5' }} className={`${block}__legend-rect`} />
        dynamic corridor
      </div>
      <div className={`${block}__legend-block`}>
        <div style={{ background: '#5da6da' }} className={`${block}__legend-rect`} />
        sample metric
      </div>
    </div>
  );
