...
export default function createHighlightedTimeframeRenderer(container, glassPane, canvas, scale) {
  ...
  applyButtonContainer.classList.add('in-events-timeline__apply-button-container');
  ...
  const mouseMoveSubscription = onMove(glassPane, e => {
  ...
  function update() {
    if (highlightedTimeframe) {
      const to = Math.max(
        clamp(scale.getRange(highlightedTimeframe[0])),
  ...
  function show() {
    ReactDOM.render(<ApplyButton />, applyButtonContainer);
