...
export default function Summary({ currencyCode, snapshotId, timeConfig }: DeploymentStatsProps) {
  const data = useObservable(
    () => getRawPayloadWithTimestamp(snapshotId, 'deploymentCostGraph', timeConfig),
  ...
  deploymentData
    ...
    .toArray()
    .map((key: string) => {
      metricArr.push(`deploymentCostGraph.${key}.totalCost`);
      labelArr.push(key);
    });

  return (
    <>
      <DashboardSection title={t('in-kubernetes:dashboards.kubecost.costByDeployment')}>
        <Chart
