export function draw({dataColumns, ctx, series, x, y, maxDistanceBetweenPoints, filters}) {
  ...
  function renderUntilGap(startingPoint) {
    ...
    // draw the series from back to top values
    for (let seriesIndex = series.length - 1; seriesIndex > 0; seriesIndex--) {
      endIndex = drawSerie(endIndex, startingPoint, seriesIndex);
    }

    endIndex = drawSerie(endIndex, startingPoint, 0, '#fff');

    return endIndex;
  }

  function drawSerie(endIndex, startingPoint, seriesIndex, color) {
    const s = series[seriesIndex];
    const colorToDraw = color ? color : s.color;
    ctx.beginPath();
