...
import * as focusedMomentDatePickerStore
...
export default connectTo(
  ...
  function TimelineFocusedMoment({ focusedMoment, focusedMomentXPosition, width }) {
    if (focusedMomentXPosition < 0) {
      return (
        <div className={`${block}__fixed-marker`} style={{ left: 0 }}>
          <SvgIcon type="arrow_left" width={12} height={12} color="#9fffff" />
          <TimelineTimestamp
            timestamp={focusedMoment}
    ...
    if (focusedMomentXPosition > width) {
      return (
        <div className={`${block}__fixed-marker`} style={{ left: width }}>
          <TimelineTimestamp
            timestamp={focusedMoment}
            dateStore={focusedMomentDatePickerStore}
            style={{
          ...
          />
          <SvgIcon type="arrow_right" width={12} height={12} color="#9fffff" />
        </div>
      );
    }
