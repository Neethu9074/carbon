...
const referenceTimestamps$ = combineLatest([serverTime$, bigBangTimestamp$]).map(([serverTime, bigBangTimestamp]) => {
...
export function getValidation$(timestamp$) {
  return combineLatest([referenceTimestamps$, timestamp$]).map(([{ serverTime, bigBangTimestamp }, timestamp]) => {
...
export function getTimestamp$(dateString$, timeString$) {
  return combineLatest([dateString$, timeString$]).map(([dateAsString, timeAsString]) => {
