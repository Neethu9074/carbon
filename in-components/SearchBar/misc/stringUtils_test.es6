...
import {replaceWith, getSubstringTillDotBackwards, getCursorTillNextDot} from 'in-components/SearchBar/misc/stringUtils';
...
describe('in-components/SearchBar/misc/stringUtils', () => {
  ...
  it('can replace parts of a string', () => {
    expect(replaceWith('oldie but goldie', 10, 16, 'foo')).to.deep.equal({cursorAfterInsertion: 13, string: 'oldie but foo'});
    expect(replaceWith('entity.loc', 7, 10, 'location')).to.deep.equal({cursorAfterInsertion: 15, string: 'entity.location'});
    expect(replaceWith('entity.location AND span:"jkdfks" span.ty /regex madness/ span.type', 39, 41,  'type')).to.deep.equal({cursorAfterInsertion: 43, string: 'entity.location AND span:"jkdfks" span.type /regex madness/ span.type'});
  });
});
