...
function writeTryBuildReleaseServerConfigFile(cb) {
  var config = {
    ...
    eum: {
      apiKey: 'hUD6LIQpRaeFDkvAf5X4Yg',
      domain: 'release.instana.rocks/eum/',
      retrievalDomain: 'release.instana.rocks/eum'
    },
    clientConfig: buildUtil.getDevModeConfig({
...
function startTryBuildReleaseProxy(cb) {
  buildUtil.startProxrox({
    serverName: 'release-instana.instana.io',
    port: 4000,
    root: false,
    ssi: true,
    tls: true,
...
function openTryBuildReleaseUrlInBrowser(cb) {
  buildUtil.openBrowser('https://local-instana.instana.rocks:4000');
  cb();
}
