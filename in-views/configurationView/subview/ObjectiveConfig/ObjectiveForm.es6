  ...
  return (
    <fieldset>
      <Section>
        ...
        {form.get('reductionOperation').map(field => (
          <FormGroup>
            ...
            <select
              id="objective-reductionOperation"
            ...
            {form.get('thresholds').map(field =>
              field.value.map((threshold, i) => (
                <div key={`threshold_${i}`}>
                  ...
                  <FormGroup>
                    <Label className={`${block}__threshold_label`} htmlFor={`objective-threshold_${i}_value`}>
                      Value
                    </Label>
                    <Input
                      id={`objective-threshold_${i}_value`}
                      type="number"
                      pattern="\d*"
                  ...
                  <FormGroup>
                    <Label className={`${block}__threshold_label`} htmlFor={`objective-threshold_${i}_severity`}>
                      Severity
                    </Label>
                    <select
                      id={`objective-threshold_${i}_severity`}
                    ...
                    >
                      ...
                      <option key="change" value="0">
                        change
                      </option>
                      <option key="warning" value="5">
                        warning
                      </option>
                      <option key="critical" value="10">
                        critical
                      </option>
                    </select>
                    {severityMessages[i]
                  ...
                  <FormGroup>
                    <Label className={`${block}__threshold_label`} htmlFor={`objective-threshold_${i}_message`}>
                      Message
                    </Label>
                    <Input
                      id={`objective-threshold_${i}_message`}
                      type="text"
      ...
      {form.get('thresholds').value.size < 1
        ? <Section>
            <Button kind="info" onClick={onAddThreshold}>
              Add threshold
            </Button>
          </Section>
        : null}
