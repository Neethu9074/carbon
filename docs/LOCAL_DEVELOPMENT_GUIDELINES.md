...
Tasks are defined in the `package.json`. They can be executed via `yarn run <taskname>`.
 ...
 - `yarn run dev`: Starts the development mode. This is what you want most of the time. The development mode will ask you several questions during startup. When you are just starting out you should always select the default options, i.e. always hit `return` to accept the defaults. If you are starting against a local backend, choose the appropriate option and then select `localhost`.
