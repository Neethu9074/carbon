...
- `yarn dev`: Starts the development mode. This is what you want most of the time. The development mode will ask you
  several questions during startup. When you are just starting out you should always select the default options, i.e.
  always hit `return` to accept the defaults. If you are starting against a local backend, choose the appropriate option
  and then select `localhost`.
- `yarn debug`: Starts debugging mode. This is essentially development mode, but with high quality sourcemaps for an
  improved debugging experience. Since generating these sourcemaps take a bit more time on the first build than it would
  in development mode, there's this dedicated debug script
- `yarn storybook`: Starts up our storybook/component catalog locally.
- `yarn test`: Executes the test suite and lints the whole source code.
- `yarn test:unit:watch`: Re-executes the whole test suite on file change. Most of the time this is too slow for a good
  TDD flow. You may want to temporarily adapt the file watch configuration from `packages/**/*_test.js` to,
  e.g., `packages/in-foobar/my_test.js`, to only re-execute a specific test or the tests in a specific sub-directory.
- `yarn build`: Builds the whole source code. This is not required for most local development workflows. *You probably
  don't need this.*
- `yarn try-build`: Can be executed after a successful `yarn run build` to start up the UI in a way that is similar to
  production deployments. The UI will expect that backend components are available locally using their default
  development ports, e.g. the ports opened via our tunnel script. *You probably don't need this.*
- `yarn check:licenses`: Generates a `ui-client-license-report.csv` and a `ui-server-license-report.csv` in the
  repository root which comes in handy when we need to validate licenses of our dependencies, e.g., for a technical due
...
the [backend repository](https://github.ibm.com/instana/backend/tree/develop/e2e-tests/ui#readme). This was done in
preparation for an eventual merge of the backend and UI repositories into a real *product* repository.

## Visual Studio Code Integration

We provide ready-made launch configurations for Visual Studio Code users (in `.vscode/launch.json`) that you can
leverage for better development experiences. You can leverage these to start the client/browser unit tests and the
server unit tests from the' Run' menu. Thanks to VS Code's great out-of-the-box integration, you can also set
breakpoints to debug your JavaScript -or- to enter a test-driven-development flow.

Also see our screencast
about [test execution and debugging in Visual Studio Code](https://drive.google.com/file/d/1JVkTjXGOhqQG4DK8OWEKaCIkf-t2Lki7/view?usp=sharing)
.
...
**Are you just starting out with local UI development? If so, skip this section for now and consider returning at a
later time.**
...
This might be due to an incompatibility between Proxrox and MacOS' default openssl executable. Check `openssl version`,
if it says something like `LibreSsl 2.xx`, consider doing `brew install openssl`/`brew upgrade openssl` and (important!)
adding its path to your shell's init scripts (`export PATH="/usr/local/opt/openssl/bin:$PATH"`). After
that, `openssl version` should say something like `OpenSSL 1.0.2o 27 Mar 2018`.
...
- `BUILD_MODE`:
  ...
  - If this is set to `ask` (non case-sensitive), you will be asked and can choose between production mode or
    development mode.
