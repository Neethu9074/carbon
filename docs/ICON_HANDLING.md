...
We have a registry will all icons (packages/in-components/SvgIcon/registry.js) plus all the plugin icons which are defined inside the "iconPath.js" file for each plugin. Newer icons are prefixed with a "lib\_". This allows us to decide in the code which dimensions to use (https://github.com/instana/ui-client/blob/develop/packages/in-components/SvgIcon/registryTranslator.js#L21). When you want to use an icon, you can simply write:
...
After this, you have to run yarn run translateIconRegistry on the console. Thiyarn s creates a new entry in the packages/in-components/SvgIcon/registry.json file. Now you can use the icon like:

```javascript
<SvgIcon type="lib_my_new_awesome_icon" width={24} height={24} />
```
