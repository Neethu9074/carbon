...
module.exports.RStats = function RStats(settings) {
	...
	function Graph(_dom, _id, _def) {
		...
		function _draw(v, alarm) {
			...
			_ctx.drawImage(_canvas, 1, 0, _canvas.width - 1, _canvas.height, 0, 0,
				_canvas.width - 1, _canvas.height);
			if (alarm) {
				_ctx.drawImage(_alarmCanvas, _canvas.width - 1, _canvas.height -
					_current * _canvas.height / _max - _elHeight);
			} else {
				_ctx.drawImage(_dotCanvas, _canvas.width - 1, _canvas.height - _current *
					_canvas.height / _max - _elHeight);
			}
		}
	...
	function PerfCounter(id, group) {
		...
		function _draw() {
			...
			var a = (_def && ((_def.below && _value < _def.below) || (_def.over &&
				_value > _def.over)));
