...
class HarmonicSphericalLayouter {
  ...
  constructor(maxElements) {
    this.maxElements = maxElements;
		...
		this.freeFields = [ {x: 0, y: 0} ]; //startPosition => (0, 0)
  ...
  getNext() {
    ...
    /* add the surounding fields if possible as new free fields
        ...
        |__6__|__7__|__8__|
    */
    this.tryAdd( { x: next.x - 1, y: next.y + 1 } );   //1
    this.tryAdd( { x: next.x, y: next.y + 1 } );        //2
    this.tryAdd( { x: next.x + 1, y: next.y + 1 } );  //3
    this.tryAdd( { x: next.x - 1, y: next.y } );         //4
    this.tryAdd( { x: next.x + 1, y: next.y } );        //5
    this.tryAdd( { x: next.x - 1, y: next.y - 1 } );    //6
    this.tryAdd( { x: next.x, y: next.y - 1 } );         //7
    this.tryAdd( { x: next.x + 1, y: next.y - 1 } );   //8

    //remove taken xy from free fields
    _.remove(this.freeFields, i => (i.x === next.x && i.y === next.y));
    //and add to blocked
    blocked.push( { x: next.x, y: next.y } );
