...
exports.GroundSpaceControl2D = function GroundSpaceControl2D(
  dimension, gapSize) {
  this.width = dimension;
  this.height = dimension;
  this.gap = gapSize === undefined ? 5 : gapSize; //the gap between the cubes
    ...
    for (var y = -this.gap; y < height + this.gap; y++) {
      ...
      if (from + x < 0 || to + y < 0) {
        continue;
			...
			if (xPos >= this.grid.width || yPos >= this.grid.height) {
        continue;
      }
  ...
  if (this.gap > 0) {
    ...
    for (x = -gapSizeForPath; x <= width + gapSizeForPath; x++) {
      for (y = -gapSizeForPath; y <= height + gapSizeForPath; y++) {
        ...
        if (xPos >= this.grid.width ||
          yPos >= this.grid.height) {
          continue;
        }
