...
describe('layoutingStorage', () => {
  ...
  const nodePositions$ = create().startWith(Immutable.fromJS(testData)).freeze();
  ...
  beforeEach(() => {
    ...
    mod = proxyquire('in-map/src/services/process/logicalLayouterStorage', {
      'in-map/src/stores/process/logicalLayouterStore': {
        nodePositions$,
        changePosition,
        removeId
      }
