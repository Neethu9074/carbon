...
export default class OrderByType {

  constructor() {}

  applyLayout(inventar) {

    const incomingExits = inventar.nodes.filter(node => node instanceof NodeUnknownService);
    const simple = inventar.nodes.filter(node => node instanceof NodeCluster);
