...
function createRow(process, id, context) {
  return [
    ...
    <PidNameColumn pid={process.get('pid')} name={process.get('name')} host={context.snapshot} />,
    <td>{percentageZeroDecimalPlaces(process.get('cpu'))}</td>,
    <td>{bytesTwoDecimalPlaces(process.get('memory'))}</td>
  ];
}

const PidNameColumn = connectTo(
  props => {
    return {
  ...
  function PidNameColumn({ name, processSnapshot }) {
    if (processSnapshot) {
      return (
        <td>
          <HierarchicalLink snapshotId={processSnapshot.get('id')} calculateHierarchy kind="dark">
