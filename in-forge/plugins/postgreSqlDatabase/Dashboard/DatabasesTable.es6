...
function createRow(db, i, context) {
  return [
    <td>{db}</td>,
    <Mtd metric={'databases.' + db + '.queries'} snapshot={context.snapshot} formatter={activityZeroDecimalPlaces} />,
    <Mtd
      metric={'databases.' + db + '.xact_commit'}
      snapshot={context.snapshot}
      formatter={activityZeroDecimalPlaces}
    />,
    <Mtd
      metric={'databases.' + db + '.xact_rollback'}
      snapshot={context.snapshot}
      formatter={activityZeroDecimalPlaces}
    />,
    <Mtd
      metric={'databases.' + db + '.blks_hit_rate'}
      snapshot={context.snapshot}
      formatter={hitRateZeroDecimalPlaces}
    />,
    <Mtd metric={'databases.' + db + '.conflicts'} snapshot={context.snapshot} formatter={zeroDecimalPlaces} />,
    <Mtd metric={'databases.' + db + '.idx_tup_read'} snapshot={context.snapshot} formatter={zeroDecimalPlaces} />,
    <Mtd metric={'databases.' + db + '.idx_tup_fetch'} snapshot={context.snapshot} formatter={zeroDecimalPlaces} />
  ];
}
      ...
      <DashboardSection title="Transactions">
        <ChartWithLegend
          ...
          y1={{
            min: 0,
            formatter: activityZeroDecimalPlaces,
            metrics: ['databases.' + db + '.xact_commit'],
          ...
          y2={{
            min: 0,
            formatter: activityZeroDecimalPlaces,
            metrics: ['databases.' + db + '.xact_rollback'],
      ...
      <DashboardSection title="Cache">
        <ChartWithLegend
          ...
          y1={{
            min: 0,
            max: 1,
            metrics: ['databases.' + db + '.blks_hit_rate'],
      ...
      <DashboardSection title="Conflicts">
        <ChartWithLegend
          ...
          y1={{
            ...
            metrics: ['databases.' + db + '.conflicts'],
      ...
      <DashboardSection title="Tuples">
        <ChartWithLegend
          ...
          y1={{
            min: 0,
            formatter: activityZeroDecimalPlaces,
            metrics: ['databases.' + db + '.idx_tup_read', 'databases.' + db + '.idx_tup_fetch'],
