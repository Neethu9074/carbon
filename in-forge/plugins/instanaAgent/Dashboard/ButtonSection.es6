...
function ImageButton({ className, children, iconType, onClick }) {
  return (
    <Button className={`${block}__button` + (className ? ` ${className}` : '')} onClick={onClick} size="sm">
      <SvgIcon className={`${block}__icon`} type={iconType} width={14} height={14} color="#172429" />
      {children}
    </Button>
  );
}
