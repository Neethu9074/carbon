...
function createRow(keyspace, index, context) {
  return ([
    <td>{keyspace}</td>,
    <Mtd metric={'keyspace.' + keyspace + '.reads'}
         snapshot={context.snapshot} />,
    <Mtd metric={'keyspace.' + keyspace + '.readLatency'}
        snapshot={context.snapshot}
        formatter={muSecondsFormatter} />,
    <Mtd metric={'keyspace.' + keyspace + '.writes'}
         snapshot={context.snapshot} />,
    <Mtd metric={'keyspace.' + keyspace + '.writeLatency'}
        snapshot={context.snapshot}
        formatter={muSecondsFormatter} />,
    <Mtd metric={'keyspace.' + keyspace + '.ssTables'}
         snapshot={context.snapshot} />,
    <Mtd metric={'keyspace.' + keyspace + '.diskSize'}
         snapshot={context.snapshot}
  ...
  return (
    <ChartWithLegend snapshotId={context.snapshot.get('id')}
           ...
           y1={{
             ...
             metrics: [
               'keyspace.' + keyspace + '.readLatency',
               'keyspace.' + keyspace + '.writeLatency'
             ],
             labels: [
           ...
           y2={{
             min: 0,
             metrics: [
               'keyspace.' + keyspace + '.reads',
               'keyspace.' + keyspace + '.writes'
             ],
             labels: [
               'Reads',
               'Writes'
