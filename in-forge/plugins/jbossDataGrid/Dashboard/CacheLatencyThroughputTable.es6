...
function createRow(cacheInfo, cacheName, context) {
  return [
    <td>{cacheName}</td>,
    <Mtd
      metric={'cachesStatistics.' + cacheName + '.averageReadTime'}
      snapshot={context.snapshot}
      formatter={msZeroDecimalPlaces}
    />,
    <Mtd
      metric={'cachesStatistics.' + cacheName + '.averageRemoveTime'}
      snapshot={context.snapshot}
      formatter={msZeroDecimalPlaces}
    />,
    <Mtd
      metric={'cachesStatistics.' + cacheName + '.averageWriteTime'}
      snapshot={context.snapshot}
      formatter={msZeroDecimalPlaces}
    />,
    <Mtd
      metric={'cachesStatistics.' + cacheName + '.throughput'}
      snapshot={context.snapshot}
  ...
  return (
    <div>
      ...
      <ChartWithLegend
        ...
        y1={{
          formatter: zeroDecimalPlaces,
          metrics: ['cachesStatistics.' + cacheName + '.throughput'],
