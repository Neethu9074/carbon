...
export default function ConnectorsTable({ snapshot, timeframe }) {
  const connectors = snapshot.getIn(['data', 'connector-config'], emptyMap).filter(c => !c.get('executor'));
...
function createRowWithConnections(connector, name, context) {
  return [
    <td>{name}</td>,
    <Mtd metric={'connectors.' + name + '.threads'} snapshot={context.snapshot} />,
    <Mtd metric={'connectors.' + name + '.threadsBusy'} snapshot={context.snapshot} />,
    <td>{connector.getIn(['threads', 'max'])}</td>,
    <Mtd metric={'connectors.' + name + '.connections'} snapshot={context.snapshot} />,
  ...
  return (
    <ChartWithLegend
      ...
      y1={{
        ...
        labels: [name + ' Threads', name + ' Threads Busy', name + ' Connections'],
        type: 'line'
      }}
    />
  );
...
function createDetails(connector, name, context) {
  return (
    <ChartWithLegend
      ...
      y1={{
        metrics: ['connectors.' + name + '.threads', 'connectors.' + name + '.threadsBusy'],
        labels: [name + ' Threads', name + ' Threads Busy'],
        type: 'line'
      }}
    />
  );
