...
export default connectTo(props => {
  ...
  instances.forEach(instance => {
    ...
    if (serviceId) {
      items.push(
        <DescriptionItem title={<HumanReadablePluginName snapshotId={serviceId}
                                                         fallback='Service' />}
                         id={serviceId}
                         key={serviceId}
                         addSeparator={true}>
    ...
    items.push(
      <DescriptionItem title={<HumanReadablePluginName snapshotId={serviceInstanceId}
                                                       fallback='Service Instance' />}
