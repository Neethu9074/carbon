...
export default function SwitchableViewHeader({ snapshot }) {
  ...
  const breadcrumbs = translatedContext
    .map(ctx => {
      return <Breadcrumb key={ctx.label} label={ctx.label} />;
    })
    .reduce((prev, curr) => [
      prev,
      <span className={chevron}><SvgIcon type="chevron_right" height={9} color="#fff" /></span>,
      curr
    ]);
